...
exports.getIdentificators = function(body, eventGetIdentificators){
    var id = body.id_alghoritm;
    console.log(id);
    var arrayIdentificators = [];
    var list_identificators ="";
    get_ident(id);
    function get_ident(id){
        connection.query('SELECT * FROM text_alghoritms WHERE id_alghoritm = '+id,finish);
    }
    function finish(err,result){
        if (err || result.length==0){
            eventGetIdentificators.emit('error');
            return;
        }
        console.log(result);
       list_identificators = result[0].list_indetificators;
        arrayIdentificators = list_identificators.split(',');
        eventGetIdentificators.emit('ok', arrayIdentificators, result[0].formula);
    }
};
