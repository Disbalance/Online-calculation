...
exports.addCatalog = function (data, eventAddCatalog){
    var body = data;
    var id_catalog;
    check_catalog(data.name);
    function check_catalog(name){
        connection.query('SELECT name FROM catalogs WHERE name = '+"'"+name+"'"+' AND id_pred='+body.id_pred, addCatalog)
    }
    function addCatalog(err, result){
        if (err || result.length != 0){
            eventAddCatalog.emit('error');
            return;
        }
        connection.query('INSERT INTO catalogs SET ?', body, finish);
    };
    function finish(err, result){
        if(err) {
            eventAddCatalog.emit("error");
...
exports.addAlghoritm = function (data, eventAddAlghoritm){
    ...
    function check_algorithm(name){
        connection.query('SELECT name FROM alghoritms WHERE name ='+"'"+name+"'"+' AND id_catalog = '+body.id_catalog, addAlghoritm)
    }
